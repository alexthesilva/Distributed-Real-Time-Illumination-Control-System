...
def monitor_and_plot(port='COM4', baud_rate=115200, duration=60):
    ...
    for timestamp, linha in data_points:
        ...
        try:
            ...
            energy_setpoint.append(float(valor_setpoint_str))
            energy_received.append(float(valor_received_str))
            tempos.append(timestamp)
        except ValueError:
            print(f"Erro ao fazer parsing da linha: {linha}")

    # Cria o título do gráfico com os parâmetros recebidos
    titulo = (f"Monitor Serial: Energy in 1 min\n"
    ...
    # Plot dos resultados com eixo X em segundos
    plt.figure(figsize=(10, 5))
    plt.plot(tempos, energy_received, 'b-', label='Energy Received')
    plt.xlabel('Tempo (s)')
    plt.ylabel('Energy')
    plt.title(titulo)
    plt.legend()
