...
def monitor_and_plot(port='COM4', baud_rate=115200, duration=60):
    ...
    for timestamp, linha in data_points:
        linha = linha.strip('$')
        try:
            valor_setpoint_str, valor_received_str = linha.split(',')
            visibility_setpoint.append(float(valor_setpoint_str))
            visibility_received.append(float(valor_received_str))
            tempos.append(timestamp)
        except ValueError:
            print(f"Erro ao fazer parsing da linha: {linha}")

    # Cria o título do gráfico com os parâmetros recebidos
    titulo = (f"Monitor Serial: Visibility Received vs Visibility Setpoint\n"
    ...
    plt.plot(tempos, visibility_received, 'b-', label='Visibility Received')
    plt.plot(tempos, visibility_setpoint, 'r--', label='Visibility Setpoint')
    plt.xlabel('Tempo (s)')
